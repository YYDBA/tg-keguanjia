...
> 跨境/外贸卖家的 Telegram 客户·订单管理 Bot。零资金部署：Vercel 免费层 + Supabase 免费层 + TON/USDT 自动收款。

## ✨ 快捷功能

- **转发即归档**：把客户消息直接转发给 bot → 自动保存，点按钮归档到客户
- **智能速记**：直接输入"Lily 订 100 个杯子 USD 2400" → 自动记单
- **客户资料卡**：`/customer 名字` 一卡聚合 订单/标签/备注/消息/跟进
- **每日待办**：每天 09:00（北京时间）自动推送 待催款/待发货/超期跟进
- **每周周报**：每周一 09:00 自动推送上周经营汇总
- **订单确认消息**：一键生成英文确认/发货通知，复制发给海外客户
- **TON/USDT 自动收款**：买家 `/pay` → 点「一键打开钱包支付」按钮（Tonkeeper 深链，预填金额+备注）→ 转账 → bot 检测链上到账 → 自动开通 Pro
